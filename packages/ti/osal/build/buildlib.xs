 ...
 *
 * DESCRIPTION: 
 *  This file contains common routines that are used by the various OSAL
 *  components.
 *
...
function createLibMake(os, device, objExtDir, makelibname,targetname, objectPath)
{
   var tooldir;
   var cmdprefix;
   var targetDir;
   ...
   switch(stringname)
   {
    ...
    if ((targetname.name == "A15F") || (targetname.name == "A9F") || (targetname.name == "A8F"))
    {
        ...
        libmakefile.writeLine("OBJDIR =  ./obj/obj_" +targetname.suffix +"/" + os.toString() + "/" + device.toString() + "/" + targetDir +"/obj" + "/" + objExtDir); 
 ...
 **************************************************************************/
var makefilelocal;
function buildLibrary (os, device, libOptions, libName, target, libFiles) 
{
    var targetDir;
    var objExtDir;
	...
	{
	    targetDir = device + "/" + targetDir;
        objExtDir = os;
	}
    
    var lldFullLibraryPath = "./lib/" + os + "/" + targetDir +"/" + libName;
    var lldFullBuildPath = "./build/" + os + "/" + targetDir +"/" + libName;
    var lldFullLibraryPathMake = "$(LIBDIR)/" + os + "/" + targetDir +"/" + libName;
    ...
    var makefilelib= createLibMake(os, device, objExtDir, lib+".mk",target,objectPath);  
    ...
    makefilelib.writeLine("\tif [ ! -d $(LIBDIR)/"+ os + "/" + targetDir+" ]; then $(MKDIR) $(LIBDIR)/" + os + "/" +targetDir+" ; fi;"); 
    ...
    Pkg.makeEpilogue += "\t$(RMDIR) " + "$(LIBDIR)/" + os + "/" + targetDir + "/ \n\n";
