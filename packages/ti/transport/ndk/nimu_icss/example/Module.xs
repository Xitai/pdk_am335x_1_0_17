/******************************************************************************
 * FILE PURPOSE: NIMU_ICSS transport unit example files.
 ******************************************************************************
 * FILE NAME: module.xs
 *
 * DESCRIPTION: 
 *  This file contains the module specification for NIMU_ICSS  transport example files.
 *
...
function modBuild() 
{
    ...
    var mkFiles = libUtility.listAllFiles (".bld", "example", true);
    for (var k = 0 ; k < mkFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = mkFiles[k];
}
